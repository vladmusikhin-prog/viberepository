...
**Версия документа:** 0.2.0 (MVP, апрель 2026)  
...
**Режим `SIGNAL_SOURCE=demo`:** синтетический worker без внешнего API (для отладки UX).
...
| Демо live-сигналы по таймеру (worker) | Да (`SIGNAL_SOURCE=demo`) |
...
└── src/
    ...
    ├── models/
    └── workers/         # SignalWorker — Polymarket или демо-режим
└── tests/
```
...
| `DEMO_LIVE_MIN_INTERVAL_SEC` | Нет (по умолчанию `3600`) | Минимум секунд между **демо** live-уведомлениями на одного пользователя (анти-спам) |
| `SIGNAL_SOURCE` | Нет (`polymarket`) | `polymarket` — реальные крупные сделки через Data API; `demo` — синтетический worker |
...
### Демо (`SIGNAL_SOURCE=demo`)

Циклически перебирает категории, шлёт шаблонные алерты с кулдауном `DEMO_LIVE_MIN_INTERVAL_SEC` на пользователя.

Ранее в демо один и тот же текст мог повторяться из‑за нового UUID на каждый тик; для демо добавлены разные шаблоны по категории и кулдаун.
