...
**Версия документа:** 0.3.1 (MVP, май 2026)  
...
| Метрика: получено сигналов `N` в `/settings` | Да |
...
При запуске `python -m src.main` или `./run.sh` берётся **неблокирующая файловая блокировка** `Bot/data/.bot_singleton.lock` (POSIX `flock`). Если бот из **этого же каталога** уже запущен, второй процесс сразу завершится с сообщением в stderr.

После `git pull` **полностью остановите** старый процесс бота (Ctrl+C в терминале, останов сервиса на сервере) и запустите снова из каталога `Bot/` — иначе в чат может уходить **старый код** со старыми кнопками. Если на **новом** ответе на `/start` всё ещё две кнопки, с большой вероятностью запущен **другой** деплой/клон с тем же токеном.
...
- При `PERSISTENCE_MODE=sqlite` счётчик и подписки хранятся в БД; при `memory` — только в памяти процесса (**сброс при рестарте**).
...
| 0.2.6 | Убран тестовый whale-alert из онбординга; отдельное пояснительное сообщение до активации без демо |
| 0.2.7 | Убраны кнопки «Полезно»/«Не полезно», `FeedbackService` и метрика `M` |
| 0.2.8 | Удалён обработчик `feedback:*`; старые сообщения с такими inline-кнопками могут показывать ошибку клиента Telegram до истечения таймаута |
| 0.2.9 | Убрана вторая CTA на `/start`; `how_it_works` в callback — только для старых сообщений в чате |
| 0.3.0 | При старте в лог пишется проверка `/start` keyboard (одна CTA); подсказка при втором процессе с тем же `BOT_TOKEN` |
| 0.3.1 | Файловая блокировка `data/.bot_singleton.lock`: второй `python -m src.main` из того же `Bot/` не стартует |
