...
**Версия документа:** 0.1.1 (MVP, апрель 2026)  
...
Почему раньше мог приходить **один и тот же текст много раз**:

- каждый тик создавался **новый** `signal_id` (`live-<uuid>`), поэтому дедупликация по паре «сигнал + пользователь» не блокировала повтор **одинакового шаблона**;
- текст рынка был **один захардкоженный** (BTC), а строка «Категория» бралась из ротатора — возможна визуальная несостыковка (например, BTC + Politics).

Сейчас: для демо заданы **разные шаблоны рынка по категории** и **кулдаун** `DEMO_LIVE_MIN_INTERVAL_SEC` между демо-push на пользователя.
