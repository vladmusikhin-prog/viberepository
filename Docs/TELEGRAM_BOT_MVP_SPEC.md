...
7. На любом из экранов, кроме самого `/start`, может нажать `Главное меню` и вернуться к стартовому выбору.
8. В `/settings` управляет категориями и видит персональную статистику полезности.
