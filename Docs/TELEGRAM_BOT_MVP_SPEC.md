...
- Получение тестового сигнала в том же формате, что и live.
- Получение live whale-alert сигналов по выбранным категориям.
...
4. Получает подтверждение подключения и кнопку тестового сигнала.
5. Получает тестовый сигнал и переходит в live-режим.
6. При live-сигнале может:
   - отметить полезность,
...
- Бот должен поддерживать сценарий из Product Card без изменения текстов и кнопок.
...
```text
+--------------------------------------+
| Ниже тестовое уведомление ...        |
|                                      |
| [Открыть тестовый сигнал]            |
...
| Start | `Как это работает` | SignalPreview |
| CategorySelection | Выбор категории | ActivationSuccess |
| ActivationSuccess | `Получить тестовый сигнал` | SignalPreview |
| ActivationSuccess | `Изменить категории` | CategorySelection |
| SignalPreview | `Получить live-сигналы` | LiveAlert (по событию) |
...
4. Низкая perceived value:
   ...
   - мера: тестовый сигнал, кнопки фидбека, метрика `N/M` в `/settings`.
...
- Integration:
  - `/start` -> выбор категорий -> тестовый сигнал,
