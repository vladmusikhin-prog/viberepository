...
Проблемы целевой аудитории (опыт биржа/беттинг → новичок в prediction markets сейчас):
- В бирже/беттинге люди привыкли к простым “движкам внимания” (линия/цена, движение, объём, алерты), но при попытке перейти в prediction markets:
  - тяжело быстро понять, **что сейчас важно** и **где шум**,
  - нет доверенной “переводилки” терминов и качества рынка (ликвидность/спред),
  - сложнее увидеть “sharp money / whale move” среди общей ленты,
  - сложно выбрать, за кем следовать (социальное доверие и трек-рекорд).
...
### B2C
- **Потенциальные пользователи Polymarket/Kalshi**, которые **не используют платформы сейчас** (или пробовали давно), но:
  - имеют опыт **биржи** (акции/крипто) и/или **спортбеттинга/беттинга**,
  - привыкли к “odds/line movement”, алертам и быстрой реакции,
  - хотят понятный “мост” в prediction markets: что смотреть, как фильтровать шум, где реальная инфа.
- Пользователи, которым нужны приоритизированные сигналы вместо “шума” и “ручного мониторинга”.
- Аудитория social-copy: люди, которые предпочитают **следовать** за более опытными (на MVP без автоторговли).
...
- `Volume spike alerts`.
- `Whale / sharp money alerts` (крупные сделки/серии сделок) — pain #6.
...
Для целевой аудитории “из биржи/беттинга” формулируем это как:
- “steam move / sharp money”: **кто-то крупный двинул рынок**, и важно увидеть это быстро + понять, не тонкий ли рынок.
...
- “Bridge for newcomers”: короткие объяснения терминов прямо в алертах (odds/probability, volume, liquidity/spread) и пресеты порогов “как у новичка/как у продвинутого”.
...
- Отложено за пределами текущего MVP: in-app опрос полезности сигналов в Telegram (можно позже через отдельные исследования или proxy-метриками поведения).

### Наблюдения из интервью (Дмитрий) — куда смотрят “power users”

- `https://x.com/unusual_whales` — источник сигналов по необычной рыночной активности.
- Reddit `https://www.reddit.com/r/wallstreetbets/top/?t=year` — источник community sentiment/excitement по акциям.
- Lantern — упоминался как полезный инструмент в прошлом (на момент интервью недоступен).
- Гипотеза интервью: хедж-фонды мониторят Reddit-сообщества для оценки уровня retail excitement.
...
| 2. Сигналы без доверия/валидируемости | Сомневающиеся пользователи, которые “не верят в шум” | Неясно, почему сигнал сработал, и насколько он надежен | Резолв outcomes + hit-rate; объяснимый контекст в алерте и скоринг качества |
...
| 4. Отсутствие контекста “что произошло и почему важно” | Занятые пользователи (нужен смысл за секунды) | Получают число, но не получают причинно-следственный контекст | Скоринг в алерте (magnitude/speed/liquidity flags) и продуктовые метрики открытий/действий |
...
| 6. Whale / sharp money “тонет” в шуме | Бетторы/трейдеры, которые привыкли ловить “steam move” | Видят движение слишком поздно и не понимают, это инфо или тонкая ликвидность | Polymarket Data API (trades/activity) `https://docs.polymarket.com/api-reference` + Polymarket CLOB (orderbook контекст) `https://docs.polymarket.com/developers/CLOB/endpoints`; на MVP допускаем proxy через abnormal volume spike |
...
- `analyst follow rate`
- `signal precision` (доля полезных алертов по поведению: открытия, переходы в рынок, retention)
